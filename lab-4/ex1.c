...
int main(int argc, char const *argv[])
{
	...
	if((pids[i] = fork())<0){
		...
		int aux,count;
		close(fd[2*i+1]);
		read(fd[2*i],aux,sizeof(int)); //colocar um array aqui, mas de que tamanho?
		...
		close(fd[2*i]); //Close output of the pipe
		write(fd[2*i+1],rng,sizeof(int)); //writes to the pipe
		}
		for(j=0;j<argv[2],j++){
			close(fd[2*i]);
			close(fd[2*i+1]);
